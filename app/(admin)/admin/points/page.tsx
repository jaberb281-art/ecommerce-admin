            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex items-center gap-3">
                        <div className="rounded-lg bg-amber-50 p-2"><Coins className="h-5 w-5 text-amber-500" /></div>
                        <div>
                            <p className="text-xs text-slate-500">Total Points in Circulation</p>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex items-center gap-3">
                        <div className="rounded-lg bg-emerald-50 p-2"><TrendingUp className="h-5 w-5 text-emerald-500" /></div>
                        <div>
                            <p className="text-xs text-slate-500">Users with Points</p>
                            <p className="text-2xl font-bold text-slate-900">{usersWithPoints}</p>
                        </div>
                    </div>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex items-center gap-3">
                        <div className="rounded-lg bg-purple-50 p-2"><Gift className="h-5 w-5 text-purple-500" /></div>
                        <div>
                            <p className="text-xs text-slate-500">Rate</p>
                            <p className="text-2xl font-bold text-slate-900">10 pts / BD 1</p>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                {expandedId === user.id && (
                                    <tr key={`${user.id}-detail`}>
                                        <td colSpan={4} className="bg-slate-50 px-6 py-4">
                                            ...
                                            ) : !detailMap[user.id] || detailMap[user.id].transactions.length === 0 ? (
                                                <p className="text-sm text-slate-400">No transactions yet.</p>
                                            ) : (
                                                <div className="space-y-2">
                                                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">
                                                        Last {detailMap[user.id].transactions.length} transactions
                                                    </p>
                                                    {detailMap[user.id].transactions.map(tx => (
